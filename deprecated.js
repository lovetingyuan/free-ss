...
function main(defaultAccountOptions) {
  ...
  return Promise.all(tasks).then((accountsList) => {
    ...
    if (!accounts.length) {
      if (argv.notify) {
        notify({
          ...
          message: 'Sorry, there are no available accounts for now.',
          wait: false,
        });
      } else {
    ...
    if (defaultAccountOptions) {
      return accounts.map(({ server, port, password, method }) => {
        return {
          ...defaultAccountOptions,
          server, server_port: port, 
          password, method, timeout: 10,
        }
      })
    }
